		...
		<div class="page-wrapper"
			...
			<div class="container text-center">
				...
				<img
					...
					class="profile-pic" />
				<h3>${req.name }</h3>
				<p class="mb-1">Name: ${req.name}</p>
				<p>
					<small class="mb-1">Mobile: ${req.mobile}</small>
				</p>
				<p>
					<small>Requested at: <fmt:formatDate value="${req.time}"
							pattern="MMM dd, yyyy" />, <fmt:formatDate value="${req.time}"
							pattern="hh:mm aa" />
					</small>
				</p>
				<c:if test="${req.status eq 'Pending' }">
					<div class="text-center" id="${req.id }">
						<div id="loader" class="spinner"></div>
						<p class="text-muted mb-4 pb-4">Waiting for approval...</p>
					</div>
				</c:if>
			</div>
		</div>
